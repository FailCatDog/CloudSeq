...
const reloadTasks = async () => {
  const data = await listPlanTasksApi(WORKSPACE_ID)
...
const handleTaskRowMoved = async ({ draggedTask, targetTask }) => {
  ...
  const updates = buildSortOrderUpdates(orderedTasks, planTaskMap.value, WORKSPACE_ID)
...
const handleTaskUpdated = async ({ task }) => {
  ...
  const existing = planTaskMap.value.get(task.id)
  if (!existing) return
...
const handleTaskStatusChange = async (row, event) => {
  const existing = planTaskMap.value.get(row.id)
  if (!existing) return

  const taskStatus = Number(event.target.value)
...
const handleAssigneeChange = async (row, event) => {
  ...
  try {
    ...
    await updatePlanTaskApi(existing.id, payload)
    existing.assigneeUserId = assigneeUserId || null
