    ...
    <template v-else>
        ...
        <section class="profile-panel wb-card profile-panel--aside">
          ...
          <div class="shortcut-list">
            <RouterLink
              ...
              class="shortcut-item"
            >
              <span class="shortcut-icon" :class="item.iconClass" aria-hidden="true">
      ...
      <section class="profile-panel wb-card">
        ...
        <div class="activity-timeline">
          <article v-for="(item, index) in activityList" :key="item.title" class="activity-item">
            <span class="activity-dot" :class="{ 'activity-dot--first': index === 0 }" aria-hidden="true" />
...
const quickLinks = [
  ...
  },
  {
    ...
    label: '安全设置',
    desc: '修改密码、登录保护、设备管理',
    iconClass: 'shortcut-icon--security',
  ...
  },
  {
    ...
    label: '返回项目空间',
    desc: '继续查看任务和进度',
    iconClass: 'shortcut-icon--workspace',
...
const activityList = computed(() => [
  { title: '最近登录', desc: lastLoginText.value === '未获取' ? '暂无登录记录。' : `${lastLoginText.value} 登录过系统。` },
  { title: '账号信息', desc: `当前账号为 ${profile.value?.username || '未获取'}。` },
  { title: '个人简介', desc: profile.value?.bio || '暂无个人简介。' },
])
...
.profile-panel--aside .shortcut-list {
  flex: 1;
}
...
.shortcut-icon--security {
  color: var(--wb-tag-design-text);
  background: var(--wb-tag-design-bg);
}
...
.activity-timeline::before {
  ...
  bottom: 10px;
  width: 2px;
  background: var(--wb-timeline-line);
}
...
.activity-dot {
  ...
  border: 2px solid var(--wb-timeline-line);
}
