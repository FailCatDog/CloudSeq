...
import { toGanttTheme } from './ganttTheme'

const { loadCurrentWorkspace, requireWorkspaceId } = useWorkspace()
const { theme, setTheme } = useTheme()
const ganttTheme = computed(() => toGanttTheme(theme.value))

/** Keep jordium's localStorage key aligned so its toolbar init matches system. */
watch(
  ganttTheme,
  (next) => {
    try {
...
/** Keep system theme as source of truth if user toggles inside the gantt toolbar. */
...
.gantt-chart {
  ...
  :deep(.gantt-root.splitter-dragging *) {
    cursor: col-resize !important;
  }

  :deep(.timeline-day.today),
  :deep(.timeline-week.today),
  :deep(.timeline-hour.today) {
    background-color: var(--wb-purple);
    color: #fff;
  }

  :deep(.sub-day-column.today),
  :deep(.day-column.today) {
    border-left-color: var(--wb-purple);
    background-color: var(--wb-purple-alpha-08);
  }

  :deep(.day-column.today::before) {
    background: var(--wb-purple-alpha-06);
  }

  :deep(.day-column.today-highlight) {
    background-color: var(--wb-purple-alpha-14) !important;
    border-left-color: var(--wb-purple) !important;

    &::before {
      background: var(--wb-purple-alpha-10) !important;
    }
  }

  :deep(.today-line-year-view) {
    background-color: var(--wb-purple);
  }

  :deep(.segmented-control:hover) {
    border-color: var(--wb-purple-border);
  }

  :deep(.segmented-thumb) {
    background: var(--wb-purple);
  }

  :deep(.segmented-item:hover:not(.active)) {
    color: var(--wb-purple);
    background: var(--wb-purple-soft);
  }

  :deep(.task-list .task-row[data-task-id='draft-row']) {
    background: var(--wb-purple-soft) !important;
    cursor: default;
  }
  ...
  :deep(.gantt-panel-left) {
    min-width: 280px !important;
  }

  :deep(.task-list .task-row),
  :deep(.task-list-header) {
    border-left: none !important;
  }

  :deep(.task-list .task-row:not([data-task-id='draft-row'])) {
    cursor: grab;
  ...
  :deep(.task-bar:not(.parent-task):not(.resource-view)) {
  ...
  :deep(.gantt-toolbar .gantt-btn-group > .gantt-btn-group-item:first-child) {
    display: none !important;
  }
}
...
.draft-add-btn {
  width: 24px;
  height: 24px;
  border: 1px dashed var(--wb-purple-alpha-45);
  border-radius: 8px;
  background: var(--wb-purple-alpha-08);
  color: var(--wb-purple);
  font-size: 16px;
  line-height: 1;
  cursor: pointer;
  ...
  &:hover {
    background: var(--wb-purple-alpha-16);
  }
}
...
<style lang="scss">
@use './gantt-theme-overrides.scss';

/* TaskDrawer 挂载在 body，需非 scoped 全局样式 */
.drawer-overlay {
  .gantt-btn-primary,
  .btn-primary {
  ...
  .form-group:has(#task-type),
  .form-group:has(#task-progress),
  .task-form > div:has(#predecessor-select),
  .form-row:has(#task-estimated-hours) {
    display: none !important;
  }
...
body:has(.drawer-overlay) {
  .el-picker-panel .el-time-picker-input,
  .el-picker-panel .el-date-picker-footer {
    display: none !important;
  }
}
</style>
